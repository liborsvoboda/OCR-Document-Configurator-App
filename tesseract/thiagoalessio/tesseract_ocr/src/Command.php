...
class Command
{
	...
	public function __toString()
	{
		...
		foreach ($this->options as $option) {
			if ($option($version) !== '-c "batch=nochop"' && $option($version) !== '-c "makebox="'){
				$cmd[] = is_callable($option) ? $option($version) : "$option";}
			else{ 
				if ($option($version) == '-c "batch=nochop"'){$cmd[] ='batch.nochop';}
				if ($option($version) == '-c "makebox="'){$cmd[] ='makebox';}
			}
		}
	...
	public function getOutputFile($withExt=true)
	{
		...
		$hasCustomExt = ['hocr', 'tsv', 'pdf', 'box'];
		$ext = in_array($this->configFile, $hasCustomExt) ? $this->configFile : 'box';
